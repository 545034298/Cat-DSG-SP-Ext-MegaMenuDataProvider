Param(
    ...
    [string]$siteURL='https://catsysdemo.sharepoint.com/sites/fusedevrogerzhangexportmegamenu001',
    [Parameter(Mandatory = $false)]
    [string]$siteAdminUrl='https://catsysdemo-admin.sharepoint.com',
    [Parameter(Mandatory=$false)]
    [System.Management.Automation.PSCredential]$credential,
    [Parameter(Mandatory = $false)]
    [string]$scope='Site',
    [Parameter(Mandatory = $false)]
    [string]$SkipFeatureDeployment=$false
)
...
if ($SkipFeatureDeployment -eq $true) {
    ...
    $app = Add-PnPApp -Path $appFile -Scope $scope -Overwrite -SkipFeatureDeployment -ErrorAction Stop
    Publish-PnPApp -Identity  $app.Id -Scope $scope -SkipFeatureDeployment 
